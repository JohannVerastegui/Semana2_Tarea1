...
namespace Semana2_Tarea1
{
    internal class Game
    {
        ...
        public Game(int numMeleeEnemies, int numRangeEnemies)
        {
            ...
            {
                enemies.Add(new EnemyMelee(50, 10, 5)); // Cant.Vida, Cant.Daño, Cant.Armadura
            }

            for (int i = 0; i < numRangeEnemies; i++)
            {
                enemies.Add(new EnemyMelee(40, 10, 5)); // Cant.Vida, Cant.Daño, Cant.Balas
            }
        }
        ...
        public void StartGame(int playerHealth, int playerDamage)
        {
            ...
            while (player.IsAlive() && currentPlayerIndex < enemies.Count)
            {
                if (playerTurn)
                {
                    ...
                    if (enemies.Count > 0)
                    {
                        if (enemyChoice == 1 && enemies.Any(e => e is EnemyMelee))
                        {
                        ...
                        else if (enemyChoice == 2 && enemies.Any(e => e is EnemyRange))
                        {
